...
/* 3A You want to keep a description of each actor. You don't think you will be performing queries on a description, so create a column
 in the table actor named description and use the data type BLOB (Make sure to research the type BLOB,
...
SELECT 
    last_name, COUNT(last_name) AS NumofActors
FROM
    actor
GROUP BY last_name;
...
WHERE
    a.staff_id = b.staff_id
        AND a.payment_date > ('2005-08-01 00:00:00')
        AND a.payment_date < ('2005-09-01 00:00:00')
GROUP BY a.staff_id;
...
SELECT 
    COUNT(inventory.film_id) AS NumOfCopies
FROM
    inventory,
    film
WHERE
...
SELECT 
    title, COUNT(*) AS NumofTimesRented
FROM
    inventory,
    film
WHERE
...
SELECT 
    store_id, SUM(amount) AS 'Business Per Store'
FROM
    payment,
    inventory,
    rental
...
SELECT 
    name AS Category, SUM(amount) AS 'Revenue'
FROM
    payment,
    inventory,
    rental,
